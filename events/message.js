                  ...
                  guildprefix.findOne(
                    ...
                    (err, res) => {
                      if (!res) {
                        ...
                        message.channel
                          .send(
                            `Hey there, it looks like that the server prefix hasn't been saved yet. I've now saved the server prefix to my DataBase as \`${guild.prefix}\`. Try running \`${guild.prefix}help\` for a list of commands.`
                          )
                          .then((msg) => {
                ...
                if (!anti) {
                  ...
                  for (const word in blacklisted) {
                    if (
                      message.content
                        .toLowerCase()
                        .includes(blacklisted[word].toLowerCase())
                    )
                      foundInText = true;
                  }
                    ...
                    warns.findOne(
                      ...
                      async (err, data) => {
                        ...
                        if (!data) {
                          ...
                          newWarns.save();
                          message.channel.send(
                            `<:allow:793205689753010217> **${message.author.tag} has been warned**`
                          );
                          ...
                          message.author
                            .send(
                              `You have been warned by ${bot.user.tag} in ${message.guild.name} for Bad Word Usage`
                            )
                            .catch((err) => {
                              message.channel.send(
                                `Uh oh, an error has ocurred while running the command. Error: **${err}**. Make sure to report this to ${botconfig.owners
                                  .map(
                                    (o) => `**${bot.users.cache.get(o).tag}**`
                                  )
                                  .join(", or ")} asap.`
                              );
                            });
                ...
                if (!user) {
                  ...
                  if (user.xp >= nextlevelxp) {
                    ...
                    if (levelch) {
                      ...
                      levelChannel.send(
                        `GG **${message.author.tag}**, you have just advanced to level **${user.level}**!`
                      );
                    } else {
                      message.channel.send(
                        `GG **${message.author.tag}**, you have just advanced to level **${user.level}**!`
