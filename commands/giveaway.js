    ...
    if (channel) {
      try {
        ...
        if (choice.emoji.name === "✅") {
          ...
          const giveawayEmbed = new Discord.MessageEmbed()
            ...
            .setColor(color).setDescription(`
                    ...
                    **Ends At:** ${moment(response.endsOn).format(
                      "LT"
                    )}, ${moment(response.endsOn).format("LL")}
                    **React with 🎉 to enter**
                `);
        ...
        return message.channel.send(
          "Error! You took to long to react, you can no longer confirm or deny the giveaway!"
        );
      }
    } else {
...
async function getResponses(message) {
  ...
  for (let i = 0; i < prompts.length; i++) {
    ...
    if (i === 0) responses.title = content;
      ...
      if (validTime.test(content)) responses.duration = content;
      else throw new Error("Invalid time format");
    } else if (i === 3) {
      if (validNumber.test(content)) responses.winners = content;
