...
module.exports.run = async (bot, message, args) => {
  ...
  embed.setDescription(
    `${bot.user.username} settings for ${message.guild.name}`
  );
  embed.setThumbnail(
    message.guild.iconURL({ size: 2048, dynamic: true, format: "png" })
  );
  embed.setFooter(
    `${bot.users.cache.get(owners[0]).tag} was here - 29/12/2020`
  );
  ...
  guildprefix.findOne({ guildID: message.guild.id }, (err, data) => {
    ...
    mutedRole.findOne({ guildID: message.guild.id }, (err, m) => {
      if (!m) {
        ...
        embed.addField(
          "Muted Role:",
          `\`${message.guild.roles.cache.get(m.muteID).name}\``
        );
      }
      modlogs.findOne({ guildID: message.guild.id }, (err, ch) => {
        ...
        memberRole.findOne({ guildID: message.guild.id }, (err, mr) => {
          if (!mr) {
            ...
            embed.addField(
              "Member role:",
              `\`${message.guild.roles.cache.get(mr.roleID).name}\``
            );
          }
          welchannel.findOne({ guildID: message.guild.id }, (err, w) => {
            if (!w) {
              ...
              embed.addField(
                "Welcome channel:",
                `\`${message.guild.channels.cache.get(w.chID).name}\``
              );
            }
            leavechannel.findOne({ guildID: message.guild.id }, (err, lc) => {
              if (!lc) {
                ...
                embed.addField(
                  "Leave channel:",
                  `\`${message.guild.channels.cache.get(lc.chanID).name}\``
                );
              }
              weltxt.findOne({ guildID: message.guild.id }, (err, wt) => {
                if (!wt) {
                  embed.addField(
                    "Welcome message:",
                    "`Welcome **{member}**, you are member number **{memberCount}**! I hope you have a great time in **{guild}**!` (Default)"
                  );
                } else {
                ...
                leavetxt.findOne({ guildID: message.guild.id }, (err, lt) => {
                  if (!lt) {
                    embed.addField(
                      "Leave message:",
                      "`**{member}** has just left **{guild}**!` (Default)"
                    );
                  } else {
