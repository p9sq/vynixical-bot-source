  ...
  if (!message.member.hasPermission("MANAGE_GUILD")) {
    ...
    if (!args[1]) {
      message.channel.send(
        `<:deny:793205689488900136> **You didn't specify your time**`
      );
    }
    if (
      !args[1].endsWith("d") &&
      ...
      message.channel.send(
        `<:deny:793205689488900136> **You didn't the correct formatting for your time**`
      );
    }
    if (isNaN(args[1][0])) {
      message.channel.send(
        `<:deny:793205689488900136> **That is not a number**`
      );
    } else {
      mutedRole.findOne({ guildID: message.guild.id }, (err, m) => {
        member.roles.add(m.muteID);
        message.channel.send(
          `<:allow:793205689753010217> **${
            Member.tag
          } has been temporarily muted for ${ms(args[1], {
            long: true,
          })} for ${reason}**`
        );
        ...
        setTimeout(function () {
          ...
          message.channel.send(
            `<:allow:793205689753010217> **${Member.tag} has been unmuted**`
          );
        }, ms(args[1]));
      });
    }
