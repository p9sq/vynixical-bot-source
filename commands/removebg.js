  ...
  guildprefix.findOne({ guildID: message.guild.id }, (err, prefix) => {
    if (!image)
      return message.channel.send(
        `Please make sure that you are uploading an image to discord while using the **${prefix.prefix}removebg** command.`
      );
    ...
    request.post(
      ...
      async function (error, response, body) {
        ...
        message.reply(
          ...
          new Discord.MessageAttachment(body, "no-bg.png"), { allowedMentions: { repliedUser: false }}
        );
      }
    );
  });
...
module.exports.config = {
  ...
  category: "Image",
  example:
    "[`Click Me`](https://i.gyazo.com/532a268867df0d629e57d55cf62bea97.gif)",
