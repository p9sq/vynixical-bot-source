...
module.exports.run = async (bot, message, args) => {
  ...
  if (Member.bot)
    return message.channel.send(
      "The user that you have mentioned is a bot user, bot users have been ignored and won't gain xp or gain a level, please mention a proper user instead of a bot!"
    );
  userlevel.findOne(
        ...
        if (!lvl) {
          ...
          card.build().then((data) => {
            guildprefix.findOne({ guildID: message.guild.id }, (err, guild) => {
              return message.channel.send(
                `Leveling is currently disabled, do \`${guild.prefix}enablelevel\` to enable the leveling system!`,
                new Discord.MessageAttachment(data, "rank.png")
              );
            });
          });
        } else {
