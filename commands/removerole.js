  ...
  if (!message.member.hasPermission("MANAGE_ROLES")) {
    ...
    if (!member)
      return message.channel.send(
        "<:maybe:793205689153093702> **Please mention a user**"
      );
    if (!role)
      return message.channel.send(
        "<:maybe:793205689153093702> **Please specify a role name or mention a role**"
      );
    member.roles.remove(role);
    message.channel.send(
      `<:allow:793205689753010217> **Successfully removed ${role.name} from ${member.user.tag}**`
    );
