      ...
      config.findOne({ guildID: message.guild.id }, (err, lvl) => {
        if (!lvl) {
          return message.channel.send(
            `<:deny:793205689488900136> **The leveling system is currently disabled, do \`${gp.prefix}enablelevel\` then \`${gp.prefix}levelchannel #channel\`**`
          );
        } else {
          ...
          lvlch.findOne({ guildID: message.guild.id }, (err, data) => {
            if (!data) {
              ...
              message.channel.send(
                `<:allow:793205689753010217> **Successfully set the level channel to ${channel.name}**`
              );
            } else {
