...
module.exports.run = async (bot, message, args) => {
    ...
    if(!message.member.hasPermission("BAN_MEMBERS")) {
        ...
        if(isNaN(userID)) return message.channel.send("<:maybe:793205689153093702> **User ID must be a number**");
        if(userID === message.author.id) return message.channel.send("<:deny:793205689488900136> **You can't ban yourself**");
        if(userID === bot.user.id) return message.channel.send("<:deny:793205689488900136> **You can't ban me. Why?**");
...
module.exports.config = {
    name: "hackban",
    description: "Hackbans the user by id (Bans the user that isn't in the server)",
