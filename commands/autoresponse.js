...
module.exports.run = async (bot, message, args) => {
    if(!message.member.hasPermission("MANAGE_GUILD")) {
        ...
        if(!args.join(" ")) return message.channel.send("<:maybe:793205689153093702> **Please specify some text**");
        if(!args.join(" | ")) return message.channel.send("<:maybe:793205689153093702> **Please specify some text**");
        ...
        au.findOne({guildID: message.guild.id}, (err, guildData) => {
            if(!guildData) {
                ...
                newGuildData.save();
                message.channel.send(`<:allow:793205689753010217> **Successfully enabled auto response**`);
            } else {
                guildData.msg = text[0],
                guildData.response = text[1];
                guildData.save();
                message.channel.send(`<:allow:793205689753010217> **Successfully set the auto response message to ${text[1]}**`);
            }
        })
    }
}
