...
module.exports.run = async (bot, message, args) => {
  if (!args[0])
    return message.channel.send("Please specify a minecraft server ip.");
  if (!args[1])
    return message.channel.send("Please specify a minecrafy server port.");
  ...
  util
    ...
    .then((res) => {
      const embed = new Discord.MessageEmbed()
        ...
        .addField(
          "Description (With minecraft color codes)",
          res.description.descriptionText,
          true
        )
