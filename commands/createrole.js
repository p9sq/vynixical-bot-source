...
module.exports.run = async (bot, message, args) => {
  guildprefix.findOne({guildID: message.guild.id}, (err, data) => {
    ...
    if(!rName) {
      return message.channel.send("<:deny:793205689488900136> **Please specify a role name**")
    }

    if(!rColor) {
      return message.channel.send("<:deny:793205689488900136> **You did not specify a color for your role**")
    }
    if(rColor>=16777215)return message.channel.send("<:deny:793205689488900136> **That hex color is to big. Keep it between 0 and 16777215**")
    if(rColor<=0)return message.channel.send("<:deny:793205689488900136> **That hex color is to small! Keep it between 0 and 16777215**")
