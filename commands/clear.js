...
module.exports.run = async (bot, message, args) => {
  if (!message.guild.me.hasPermission("MANAGE_MESSAGES"))
    return message.channel.send(
      "Error! I am missing the `MANAGE_MESSAGES` permission!"
    );
  if (!message.member.hasPermission("MANAGE_MESSAGES")) {
    ...
    if (!args[0])
      return message.channel.send(
        "Please specify the amount of messages you want to delete!"
      );
    ...
    message.channel
      .send(
        `<:allow:793205689753010217> **Successfully purged ${args[0]} messages**`
      )
    ...
    modlogs.findOne({ guildID: message.guild.id }, (err, ch) => {
      if (!ch) return;
      embed5.addField(
        "The user which used clear command:",
        `${message.author.tag}`
      );
