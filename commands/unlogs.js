...
module.exports.run = async (bot, message, args) => {
    if(!message.member.hasPermission("ADMINISTRATOR")) {
        ...
        modlogs.findOneAndDelete({ guildID: message.guild.id} , ( err, ch ) => {
            message.channel.send(`<:allow:793205689753010217> **Successfully disabled the ${message.guild.channels.cache.get(ch.channelID).name} logs channel from ${message.guild.name}**`)
        })
    }
}
