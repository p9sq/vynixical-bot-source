...
module.exports.run = async (bot, message, args) => {
    if(!args.join(" ")) return message.channel.send("Please provide a music video title!")
    const {body} = await superagent.get(`https://some-random-api.ml/lyrics?title=${encodeURIComponent(args.join(" "))}`)
    ...
    .setColor(color)
    embed.setDescription(body.lyrics)
    embed.setTimestamp()
